...
Java_com_bamboo_bambooheli_activity_BebopActivity_car_1plate(JNIEnv *env, jobject instance, jlong imgsrc1, jlong imgsrc2, jint fnum1) {
...
//	image = imread(imgsrc);  //  Load an image file.
	Mat &image = *(Mat *) imgsrc1;
	Mat &resimage = *(Mat *) imgsrc2;
    ...
    resimage = image(Rect(boundRect2[select].tl().x - 20, boundRect2[select].tl().y - 20, plate_width + 40, plate_width*0.3));
